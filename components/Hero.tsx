...
export default function Hero() {
  ...
  /* Reloj Uruguay (UTC-3, sin DST) */
  useEffect(() => {
    const tick = () => {
      const n = new Date();
      // Uruguay = UTC-3 fijo (sin horario de verano desde 2015)
      const uy = new Date(n.getTime() - 3 * 60 * 60 * 1000);
      setTime(
        [uy.getUTCHours(), uy.getUTCMinutes(), uy.getUTCSeconds()]
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-10 lg:gap-14 items-center">
          ...
          <div>
            ...
            <div className="inline-flex items-center gap-2 border border-[var(--borderh)] rounded-[2px] px-3 py-1.5 mb-6 font-mono text-[.67rem] tracking-[.16em] text-[var(--textm)]">
              ...
              SISTEMA ACTIVO ·{" "}
              <time aria-live="polite" aria-label={`Hora Uruguay: ${time}`}>{time}</time> UYT
            </div>

            {/* H1 — tamaño reducido para mejor legibilidad */}
            <h1
              id="hero-heading"
              className="font-display font-extrabold leading-[.94] tracking-[-0.02em] text-white mb-5"
            ...
            <p className="text-[1rem] leading-[1.85] text-[var(--text)] font-light max-w-[500px] mb-8">
              Infraestructura IaaS, PaaS y SaaS para empresas que{" "}
              <strong className="text-white font-semibold">no pueden permitirse una brecha</strong>.
              Zero Trust nativo, cifrado en reposo y trazabilidad ALCOA+ certificada.
            </p>
            ...
            <dl className="flex flex-wrap gap-5 mb-10" aria-label="Indicadores clave de Bitera Digital">
              {KPIS.map((k) => (
                <div key={k.l} className="border-l-2 border-[var(--cyand)] pl-3">
                  <dt className="font-mono text-[.6rem] tracking-[.12em] text-[var(--textm)] uppercase">{k.l}</dt>
                  <dd className="font-display font-extrabold text-[1.5rem] text-[var(--cyan)] leading-tight">{k.n}</dd>
                </div>
              ))}
            </dl>
            ...
            <div className="flex flex-wrap gap-3">
              <a
                href={WA_URL}
                className="inline-flex items-center gap-2.5 font-display font-bold text-[.92rem] tracking-[.06em] px-5 py-3.5 rounded-[3px] bg-[var(--wa)] text-white hover:bg-[#1DB954] transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(37,211,102,.35)]"
              ...
              <a
                ...
                className="inline-flex items-center gap-2 font-display font-semibold text-[.88rem] tracking-[.06em] px-5 py-3.5 rounded-[3px] border border-[var(--borderh)] text-[var(--text)] hover:border-[var(--cyan)] hover:text-[var(--cyan)] hover:bg-[rgba(0,200,240,.06)] transition-all"
          ...
          >
            ...
            <div className="flex items-center gap-1.5 px-4 py-2.5 bg-[var(--bg3)] border-b border-[var(--border)]">
              ...
              <span className="flex-1 text-center font-mono text-[.65rem] tracking-[.1em] text-[var(--textm)]">
